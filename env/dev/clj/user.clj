(ns user
  ...
  (:require
   ...
   [jehaby.wpt.core :refer [start-app]]
   [jehaby.wpt.db :as db]))
...
(defn migrate []
  (migratus/init (db/->migratus-cfg state/system)))

(defn dev-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (jehaby.wpt.config/system-config {:profile :dev})
                                  (ig/expand)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (jehaby.wpt.config/system-config {:profile :test})
                                  (ig/expand)))))
